document.addEventListener('DOMContentLoaded', () => {
  ...
  // Vérifier si TodoFirebaseManager est disponible
  ...
  // ===== INITIALISATION =====
  // Vérifier l'état d'authentification et créer la sous-collection si nécessaire
  ...
  // Vérifier l'état d'authentification et initialiser Firestore si nécessaire
  function checkAuthAndInitialize() {
    ...
    if (isConnected) {
      console.log('Utilisateur connecté, initialisation de la sous-collection tasks...');
      
      // Tenter de créer explicitement la sous-collection
      if (typeof TodoFirebaseManager.ensureTasksCollection === 'function') {
        TodoFirebaseManager.ensureTasksCollection()
          .then(result => {
            console.log('Initialisation de la sous-collection tasks:', result ? 'Réussie' : 'Échouée');
          })
          .catch(error => {
            console.error('Erreur lors de l\'initialisation de la sous-collection:', error);
          });
      } else if (typeof window.ensureTasksCollection === 'function') {
        window.ensureTasksCollection()
          .then(result => {
            console.log('Initialisation de la sous-collection tasks (via window):', result ? 'Réussie' : 'Échouée');
          })
          .catch(error => {
            console.error('Erreur lors de l\'initialisation de la sous-collection (via window):', error);
          });
      }
    }
  }
  ...
  function toggleTaskCompletion(taskId, category) {
    ...
    // Si l'utilisateur est connecté, mettre à jour dans Firebase
    const isConnected = typeof TodoFirebaseManager !== 'undefined' && 
                        TodoFirebaseManager.isUserLoggedIn && 
                        TodoFirebaseManager.isUserLoggedIn();
    
    if (isConnected) {
      console.log('Mise à jour de la tâche dans Firestore...');
      
      if (task.repetition && task.repetition.total > 0) {
        // Mettre à jour la répétition
        TodoFirebaseManager.updateTaskRepetition(category, taskId, task.repetition)
          .then(result => {
            console.log('Mise à jour de la répétition dans Firestore:', result);
          })
          .catch(error => {
            console.error('Erreur lors de la mise à jour de la répétition dans Firestore:', error);
          });
      }
      ...
      TodoFirebaseManager.toggleTaskCompletion(category, taskId, task.completed)
        .then(result => {
          console.log('Mise à jour de l\'état dans Firestore:', result);
        })
        .catch(error => {
  ...
  function saveEditedTask() {
    ...
    if (isConnected) {
      ...
      TodoFirebaseManager.updateTask(category, id, task)
        .then(result => {
          console.log('Mise à jour dans Firestore:', result);
        })
        .catch(error => {
  ...
  function deleteTask(taskId, category) {
    ...
    // Si l'utilisateur est connecté, supprimer de Firebase
    const isConnected = typeof TodoFirebaseManager !== 'undefined' && 
                        TodoFirebaseManager.isUserLoggedIn && 
                        TodoFirebaseManager.isUserLoggedIn();
    
    if (isConnected) {
      console.log('Suppression de la tâche dans Firestore...');
      
      TodoFirebaseManager.deleteTask(category, taskId)
        .then(result => {
          console.log('Suppression dans Firestore:', result);
        })
        .catch(error => {
