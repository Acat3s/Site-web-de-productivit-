document.addEventListener('DOMContentLoaded', () => {
  ...
  function loadTasksFromLocalStorage() {
    ...
    // Charger toutes les tâches à partir de la clé unique 'tasks'
    const savedTasks = localStorage.getItem('tasks');
    if (savedTasks) {
      const parsed = JSON.parse(savedTasks);
      tasks.daily = parsed.daily || [];
      tasks.weekly = parsed.weekly || [];
      tasks.punctual = parsed.punctual || [];
      tasks.general = parsed.general || [];
    }
  }
  ...
  function checkAndResetDailyTasks() {
    ...
    console.log('Vérification de la réinitialisation des tâches quotidiennes...');
    console.log('Dernière réinitialisation:', lastResetDate);
    console.log('Aujourd\'hui:', today);
    
    // Si la dernière réinitialisation n'a pas eu lieu aujourd'hui, réinitialiser les tâches
    if (lastResetDate !== today) {
      console.log('Réinitialisation des tâches quotidiennes nécessaire...');
      resetDailyTasks();
      
      // Mettre à jour la date de dernière réinitialisation
      localStorage.setItem('lastDailyTasksReset', today);
    } else {
      console.log('Les tâches quotidiennes ont déjà été réinitialisées aujourd\'hui.');
    }
  }
  ...
  async function resetDailyTasks() {
    ...
    if (document.querySelector('.notification-container')) {
      ...
      notification.innerHTML = '<i class="fas fa-sync-alt"></i> Les tâches quotidiennes ont été réinitialisées.';
  ...
  function resetDailyTasksAtMidnight() {
    ...
    console.log(`Programmation de la réinitialisation des tâches dans ${Math.floor(msToMidnight/1000/60)} minutes...`);
    
    // Programmer la réinitialisation
    setTimeout(() => {
      console.log('Minuit atteint, réinitialisation des tâches quotidiennes...');
