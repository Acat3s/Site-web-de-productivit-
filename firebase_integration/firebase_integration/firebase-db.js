...
class FirestoreManager {
  ...
  static async updateWeeklyStatistics(statsData) {
    try {
      ...
      if (statsDoc.exists()) {
        ...
        const startDateStr = statsData.startDate.toISOString().split('T')[0];
        const existingIndex = weeklyCompletion.findIndex(item => 
          item.startDate.toDate().toISOString().split('T')[0] === startDateStr
        );
  ...
  // ===== GESTION DU STOCKAGE VIDÉO =====
  
  // Ajouter une nouvelle vidéo
  static async addVideo(videoData) {
    try {
      const userId = this.getCurrentUserId();
      const videosRef = collection(db, 'videoStorage', userId, 'videos');
      ...
      // Ajouter la vidéo à Firestore
      const docRef = await addDoc(videosRef, dataWithTimestamps);
  ...
  static async getVideos(filters = {}) {
    try {
      ...
      // Trier par date d'upload par défaut
